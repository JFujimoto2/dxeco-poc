...
**全体: Line 91.2% (784/860) / Branch 70.7% (188/266)**
...
| # | 項目 | 優先度 | 工数 | 効果 |
|---|------|--------|------|------|
| 1 | A1. CSVエクスポート共通化 | 高 | 0.5h | 重複90行削減 |
| 2 | B1. ダッシュボードクエリ最適化 | 高 | 1h | 8→2クエリ削減 |
| 3 | B2. N+1解消 | 高 | 0.5h | 一覧画面の高速化 |
| 4 | D1. TasksController テスト | 高 | 1h | 59.5%→90%+ |
| 5 | A2. CSVインポート基底クラス | 中 | 0.5h | 重複40行削減 |
| 6 | A3. インポートモーダル共通化 | 中 | 0.5h | 重複30行削減 |
| 7 | A4. フォームエラー共通化 | 中 | 0.5h | 重複20行削減 |
| 8 | C1. TaskBuilder抽出 | 中 | 1h | 可読性向上 |
| 9 | C3. フィルタスコープ化 | 中 | 0.5h | コントローラー簡素化 |
| 10 | D2. ApprovalRequests テスト | 中 | 1h | 76.1%→90%+ |
| 11 | D3. ApplicationHelper テスト | 低 | 0.5h | 53.3%→90%+ |
| 12 | D4. ジョブテスト拡充 | 低 | 1h | 分岐カバレッジ向上 |
| 13 | B3. インデックス追加 | 低 | 0.5h | クエリ高速化 |
| 14 | E1. トランザクション追加 | 低 | 0.5h | データ整合性 |
| 15 | E2. マジックストリング定数化 | 低 | 0.5h | 保守性向上 |
| 16 | E3. エラーログ改善 | 低 | 0.5h | 運用時のデバッグ |
| 17 | C2. SurveyCleanupTask抽出 | 低 | 1h | 可読性向上 |
