...
テストカバレッジ（RSpec 238件 + Playwright E2E 68件）とCI自動化が整備済みのため、日常的な保守コストは低い想定。
...
4. **1.4 コスト可視化** を追加（1日）🔧 次のタスク
5. **1.5 CSVエクスポート** は本番移行フェーズで対応

---

作成日: 2026年2月11日
更新日: 2026年2月14日（1.3 Entra ID SaaSアカウント同期 + パスワード期限検出 完了）
