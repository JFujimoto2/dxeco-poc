...
テストカバレッジ計測（SimpleCov）とソースコード解析の結果、17項目のリファクタリング候補を特定。

**テストカバレッジ:** Line 91.2% (784/860) / Branch 70.7% (188/266)

| カテゴリ | 項目数 | 概要 |
|----------|--------|------|
| A. コード重複解消 | 4 | CSVエクスポート共通化、インポート基底クラス、パーシャル統合 |
| B. パフォーマンス改善 | 3 | ダッシュボードクエリ最適化、N+1解消、インデックス追加 |
| C. 責務分離 | 3 | TaskBuilder抽出、フィルタスコープ化 |
| D. テストカバレッジ向上 | 4 | 低カバレッジファイルのテスト拡充（59.5%→90%+） |
| E. セキュリティ・堅牢性 | 3 | トランザクション追加、エラーログ改善 |
...
4. ~~**1.4 コスト可視化**~~ ✅ 完了
5. ~~**1.5 CSVエクスポート**~~ ✅ 完了
6. **2.4 リファクタリング** — テストカバレッジ向上 + コード品質改善

---

作成日: 2026年2月11日
更新日: 2026年2月14日（リファクタリング計画追加）
