...
テストカバレッジ（RSpec 276件 + Playwright E2E 73件）とCI自動化が整備済みのため、日常的な保守コストは低い想定。
...
| テスト/コード比 | 0.89 |
| テストカバレッジ | Line 95.5% / Branch 77.9% |
| DBテーブル数 | 13 |
...
1. **高テストカバレッジ（95.5%）** — リグレッションリスクが低く、安全にアップデート可能
...
**テストカバレッジ:** Line 91.2% → **95.5%** / Branch 70.7% → **77.9%**
**RSpec:** 253件 → **276件**
