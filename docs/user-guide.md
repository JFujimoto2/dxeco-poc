...
> **グループベース同期（推奨）:** 環境変数 `ENTRA_SYNC_GROUP_ID` にEntra IDグループのオブジェクトIDを設定すると、そのグループのメンバーのみを同期対象にできます。未設定の場合はテナント全ユーザーが同期されます。設定手順は [環境構築ガイド](environment-setup.md#グループベース同期の設定推奨) を参照。
...
| 判断 | 条件 |
| --- | --- |
| **本番移行GO** | 現場管理者から「これで業務が回る」と評価される |
| **追加開発して再トライ** | 基本は良いが特定機能の不足がある |
