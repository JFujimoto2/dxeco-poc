...
> **グループベース同期（推奨）:** 環境変数 `ENTRA_SYNC_GROUP_ID` にEntra IDグループのオブジェクトIDを設定すると、そのグループのメンバーのみを同期対象にできます。未設定の場合はテナント全ユーザーが同期されます。設定手順は [運用手順書のEntra IDセットアップ](operations.md#entra-id-セットアップsso--ユーザー同期) を参照。
