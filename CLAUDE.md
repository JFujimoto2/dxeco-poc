...
5. **ドキュメント更新** (コミット前に必ず実施)
   - `docs/plans/` 計画書のチェックリストを `- [x]` に更新
   - `docs/features/` の該当機能ドキュメントを更新
   - 実装時の補足事項があれば追記
6. **コミット & push**
...
- CI: Rubocop（lint） + Brakeman（セキュリティ） + RSpec（テスト） + Playwright E2E
...
│   └── ...                 # 一般コントローラー
├── models/
├── mailers/                # Action Mailer（タスク・承認・サーベイ通知）
...
docs/
├── plans/                  # 機能別の実装計画書
├── features/               # 機能別ドキュメント（画面・データ項目・権限等）
├── インフラ構成案.md
├── デクセコ_調査レポート_.md
├── バッチ処理設計.md
├── 自社開発_タスク_機能一覧.md
...
| 4. 仕上げ | Teams通知・メール通知・操作ログ・デモデータ・デプロイ |
