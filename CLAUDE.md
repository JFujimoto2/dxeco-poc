...
- `.env` ファイルは絶対に参照・読み取りしないこと（認証情報が含まれるため）
...
- 送信方式: Microsoft Graph API（`lib/graph_api_delivery.rb`）— Security Defaults 有効テナント対応
- デフォルト送信元: `MAILER_FROM` 環境変数（未設定時 `noreply@example.com`）
- Entra ID アプリに `Mail.Send` 権限が必要
- 開発環境: `ENTRA_CLIENT_ID` 設定時は Graph API で実送信、未設定時は `letter_opener_web`（`/letter_opener` でプレビュー）
